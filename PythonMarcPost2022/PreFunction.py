...
def read_in(_filenamelist, _LEN):
    #
    # Input： 
    ...
    for i in range(0, _LEN):
        ...
        _pp=_tempinp.addfile(_filenamelist[i])
        if _pp == 0:
...
def define_item(_stmod, _INP0, _filenamelist, judgeI):
    ...
    if (input("Is items to be print from each postfile the same?(Y/N)") == 'Y') :
        ...
        tempd = int(input("Please typein the item:"))
        for x in _filenamelist:
            _itemmodlist.append(tempd)
    else:
